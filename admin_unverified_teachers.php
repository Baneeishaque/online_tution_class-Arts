...
if (isset($_GET['action'])) {
    ...
    if ($_GET['action'] == 'verify-teacher') {

        $teacher_id = $_GET['teacher-id'];

        $teacher_update_sql = "UPDATE `teachers` SET `status`=1 WHERE `teacher_id`='$teacher_id'";

        $teacher_update_query_result = $db_connection->query($teacher_update_sql);

        if ($teacher_update_query_result == 1) {

            //Update Success
            header("Location: admin_unverified_teachers.php?message=success");
            exit();

        } else {

            //Update Failure
            header("Location: admin_unverified_teachers.php?message=failure");
            exit();
        }
    }
}

$teacher_fetch_sql = "SELECT `teacher_id`, `full_name`, `mobile_number`, `email_address`, `status`, `username`, `password`,`courses`.`course_id`,`courses`.`course_name`,`streams`.`stream_id`,`streams`.`stream_name` FROM `teachers`,`courses`,`streams` WHERE `status` = 0 AND `teaching_class`=`streams`.`stream_id` AND `streams`.`course_id`=`courses`.`course_id`";
...
<head>
    ...
    <title>TANUR ARTS COLLEGE - Unverified Teachers</title>
            ...
            <ul class="sidebar-menu" id="nav-accordion">
                ...
                <li class="sub-menu">
                    ...
                    <ul class="sub">
                        ...
                        <li class="active"><a href="admin_unverified_teachers.php">Unverified Teachers</a></li>
        ...
        <section class="wrapper">
            ...
            if (isset($_GET['message'])) {
                ...
                if ($_GET['message'] == 'success') {
                    ...
                    echo '<br>
            <div class="alert alert-success"><b>Well done!</b> Teacher Verified successfully...</div>';
                    ...
                    <div class="content-panel">
                        <table class="table table-striped table-advance table-hover">
                            ...
                            <tr>
                                ...
                                <th class="hidden-phone"><i class="fa fa-question-circle"></i> Handling Course</th>
                            ...
                            <tbody>
                            <?php
                            while ($student_fetch_query_result_row = mysqli_fetch_assoc($teacher_fetch_query_result)) {
                                ...
                                <td>
                                    <a href="admin_unverified_teachers.php?action=verify-teacher&teacher-id=' . $student_fetch_query_result_row['teacher_id'] . '"><button class="btn btn-success btn-xs"><i class="fa fa-check"></i></button></a>
                                </td>
                            </tr>';
                            }
                            ?>
